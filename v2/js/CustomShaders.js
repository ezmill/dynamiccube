var CustomShaders = function(){
	...
	this.flowShader = {
		...
		fragmentShader: [
			...
			"    for( int i=0; i<5; i++ ){",
			"    	float h  = dot( texture2D(texture, uv*0.99        ).xyz,  		vec3(0.25) );",
			"    	float h1 = dot( texture2D(texture, uv+vec2(e.x,h*mouse.y)).xyz, vec3(0.25) );",
			"    	float h2 = dot( texture2D(texture, uv+vec2(h*mouse.x,e.y)).xyz, vec3(0.25) );",
			"    	vec2 g = 0.001*vec2( (h1-h), (h2-h) )/e;",
			// "    	vec2 g = 0.001*vec2( h1*h, h2*h )/vec2(-e.x, e.y);",
			"    	vec2 f = g.xy*vec2(30.0*mouse.x, 30.0*mouse.y);",
			// "    	vec2 f = g.xy*vec2(0.0);",
	...
	this.reposShader = {
		...
		fragmentShader: [
			...
			"    vec2 offs = vec2(look.y-look.x,look.w-look.z)*vec2(mouse.x, mouse.y);",
			"    vec2 coord = offs+tc;",
	...
	this.noiseBlur =  {
		...
		fragmentShader: [
			...
			"		",
			"    float offset = time + gl_FragCoord.x/resolution.x;",
			"    ",
			"	float acc = 0.0;",
			"	vec3  col = vec3(0.0);",
			"	for( int i=0; i<10; i++ )",
			...
			"		float w = 4.0*h*(1.0-h);",
			"		",
			"		vec3 ttt = w*texture2D( texture, uv ).xyz;",
